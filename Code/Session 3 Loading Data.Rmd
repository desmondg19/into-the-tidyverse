...
```{r}
this_plot<-tidy_read_mod$plot

is.character(tidy_read_mod$plot)
is.numeric(tidy_read_mod$pot)
is.character(tidy_read_mod$ZIP_POSTAL)
is.numeric(tidy_read_mod$ZIP_POSTAL)
...
bank_branches<-here('Data',"BankBranchesData.txt")%>%
  read_tsv()
```
here function -> already on computer, tells you where to go in the directory
tsv -> type of data, tabs (how to handle the data)
```{r}
bank_branches1<-read_tsv("Data/BankBranchesData.txt")
```
